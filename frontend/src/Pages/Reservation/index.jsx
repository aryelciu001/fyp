...
export default function Reservation() {
  ...
  const [selectionOpenInfo, setSelectionOpenInfo] = useState(
    "Selection is closed"
  );
  ...
  const update = () => {
    request(ApiRequestType.GET_RESERVATION).then((res) => {
      if (unmounted) return;
      dispatch(updateReservation({ reservation: res.data }));
    });
    request(ApiRequestType.GET_SELECTION).then((res) => {
      if (unmounted) return;
      dispatch(updateSelection({ selection: res.data }));
    });
    request(ApiRequestType.GET_SELECTION_INFO).then((res) => {
      if (unmounted) return;
      if (res.data.selectionopen) {
        setSelectionOpenInfo(
          genSelectionInfo("open", res.data.selectionopentime)
        );
        setSelectionCloseInfo(
          genSelectionInfo("close", res.data.selectionclosetime)
        );
      } else {
  ...
  return (
    <React.Fragment>
      ...
      <ProjectList type="selection" list={selection} />
      <h1>Your Reservations</h1>
      <ProjectList list={reservation} update={update} type="reservation" />
    </React.Fragment>
  );
}
