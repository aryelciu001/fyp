...
UserRouter.post("/verifyemail", async function (req, res) {
  ...
  try {
    ...
    // if email is already recorded
    code = await VerificationcodeController.getCode(email);
  }

  const emailPayload = verifyEmail(code);

  Mailer.sendEmail(email, emailPayload).catch((e) => ErrorResponse(e, res));
...
UserRouter.post("/register", async function (req, res) {
  ...
  try {
    const codeIsCorrect = await VerificationcodeController.verifyCode(
      email,
      verificationCode
    );
    if (!codeIsCorrect)
      return ErrorResponse(
        new MyError(ErrorMessage.WRONG_VERIFICATION_CODE),
        res
      );
  ...
  async function (req, res) {
    try {
      ...
      users.forEach(async (user) => {
        ...
        if (existingUser) {
          promises.push(
            UserController.editUser(email, matricNumber, "", role, eligible)
          );
        } else {
          promises.push(
...
UserRouter.get("/migrate", async (req, res) => {
  try {
